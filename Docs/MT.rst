...
* ``--T1``
    ...
    Path to a T1 map (seconds). **Required** to stabilise the fitting.

* ``--lineshape, -l``

    Either Gaussian, Lorentzian, Superlorentzian, or a path to a ``.json`` file generated by ``qi lineshape``. Default is Gaussian.

* ``--f0, -f``

    Path to an off-resonance (f0) map in Hz.
...
* ``--hloss, -h``
    ...
    Huber Loss parameter for robust fitting (default 1).
...
* ``--ref, -r``
    ...
    Divide output by this reference image and multiply by 100 (output as percentage).
...
* ``--pools, -p``
    ...
    Number of Lorentzian pools to fit (default 1).
...
* ``--ref, -r``
    ...
    Path to an external reference image.
...
* ``--T2f``
    ...
    Path to a T2 Free map (used for simulation only).

* ``--G0``

    Lineshape value at resonance (default 1.4e-5).
